...
-- Sprint 010B discreet concept progression. See migration 011.
ALTER TABLE challenge_variants ADD COLUMN IF NOT EXISTS demand_stage SMALLINT NOT NULL DEFAULT 0 CHECK (demand_stage BETWEEN 0 AND 4);
ALTER TABLE challenge_variants ADD COLUMN IF NOT EXISTS context_type TEXT NOT NULL DEFAULT 'same_context' CHECK (context_type IN ('same_context','varied_representation','transfer'));
ALTER TABLE challenge_variants ADD COLUMN IF NOT EXISTS scaffold_profile TEXT NOT NULL DEFAULT 'standard' CHECK (scaffold_profile IN ('standard','reduced'));
CREATE UNIQUE INDEX IF NOT EXISTS uq_attempt_challenge_step ON attempt_challenge_state(attempt_id, step_order);
CREATE TABLE IF NOT EXISTS learner_concept_progression (
  ...
  rule_version TEXT NOT NULL, last_evidence_at TIMESTAMPTZ, updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
...
CREATE TABLE IF NOT EXISTS concept_evidence_windows (
  ...
  correct BOOLEAN NOT NULL, independent_attempt_recorded BOOLEAN NOT NULL, paper_completed BOOLEAN NOT NULL,
  ...
  rule_version TEXT NOT NULL, recorded_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
...
CREATE TABLE IF NOT EXISTS concept_progression_history (
  ...
  movement TEXT NOT NULL CHECK (movement IN ('up','down')), reason TEXT NOT NULL,
  trigger_evidence_id BIGINT NOT NULL REFERENCES concept_evidence_windows(id) ON DELETE RESTRICT,
  rule_version TEXT NOT NULL, recorded_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
CREATE INDEX IF NOT EXISTS idx_concept_evidence_learner_concept ON concept_evidence_windows(learner_id, concept_id, recorded_at DESC, id DESC);
CREATE INDEX IF NOT EXISTS idx_progression_history_learner_concept ON concept_progression_history(learner_id, concept_id, recorded_at DESC, id DESC);

-- Sprint 012 pilot operations and parent-entered factual observations. See migration 012.
...
CREATE INDEX IF NOT EXISTS idx_pilot_sessions_parent_created ON pilot_sessions(parent_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_pilot_sessions_learner_created ON pilot_sessions(learner_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_pilot_observations_session_created ON pilot_observations(session_id, created_at DESC, id DESC);
COMMENT ON TABLE pilot_observations IS 'Parent-entered factual pilot observations. Do not store raw learner answers, diagnostic labels, sibling comparisons, or hidden progression state.';
